...
def _summarize_topic_result(result):
    ...
    return {
        "call_order": result.get("call_order", []),
        "resolved_topic": result.get("resolved_topic"),
        "sentence_level_narrative_shifts": shifts,
        "top_topic_sentences": result.get("top_topic_sentences", []),
    }
...
def _run_single_config(
    ...
    evaluation_metrics = run_evaluation_and_save(model, topic_window_data, run_config, device)
    run_summary_path = save_training_artifacts(run_config, training_history, evaluation_metrics)
    ...
    return {
        ...
        "model_paths": {
            "best": run_config["model_best_path"],
            "last": run_config["model_last_path"],
            "evaluated": run_config["model_evaluated_path"],
        },
...
def run_grid_training_and_testing():
    output_root = Path("./Approach_lambda_output")
    ...
    with open(summary_path, "w", encoding="utf-8") as file:
        json.dump(_to_serializable(summary_payload), file, indent=2)
