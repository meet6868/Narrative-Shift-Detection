#!/usr/bin/env python3
"""Evaluate Approach2 narrative shift predictions against ground truth files.
...
This script is strict Approach2 mode:
- Ground truth root is hardcoded to: newinput
- Prediction root is hardcoded to: Approach2_output
- It auto-discovers all Approach2 configs from filenames and evaluates each config.
- It saves full evaluation output to: approach2_eval.json
"""
...
OUTPUT_ROOT_DIRNAME = "Approach2_output"
OUTPUT_JSON_FILENAME = "approach2_eval.json"
APPROACH2_PATTERN = re.compile(
    r"^(?P<prefix>.+?)_day_gap_(?P<fg>\d+)_(?P<dg>\d+)_w(?P<w>\d+)_s(?P<s>\d+)_t(?P<t>\d+p\d+)_Ner(?P<ner>\d+)_(?P<difficulty>hard|medium|low)_results\.json$",
    re.IGNORECASE,
)
...
def _discover_approach2_configs(
    output_dir: Path,
    input_root: Path,
    allow_incomplete_configs: bool = False,
) -> List[Dict[str, Any]]:
    """Discover Approach2-style configs from output filenames and build approach entries."""
    ...
    for pred_file in sorted(output_dir.glob("*_results.json")):
        match = APPROACH2_PATTERN.match(pred_file.name)
        if not match:
            continue
        matched_file_count += 1
        prefix = match.group("prefix")
        fixed_group_size = match.group("fg")
        max_day_gap = match.group("dg")
        ...
        grouped[(prefix, fixed_group_size, max_day_gap, window, stride, temperature_token)][difficulty][ner_id] = str(
            pred_file.resolve()
        )

    if matched_file_count == 0:
        raise ValueError(
            f"No prediction files matched Approach2 naming convention in {output_dir}"
        )
    ...
    for (
        ...
        stride,
        temperature_token,
    ), coverage in sorted(grouped.items()):
        ...
        label = (
            f"{prefix}_fg{fixed_group_size}_dg{max_day_gap}_"
            f"w{window}_s{stride}_t{temperature}"
        )
        ...
        discovered.append(
            {
                "name": label,
                "hyperparameters": {
                    "fixed_group_size": int(fixed_group_size),
...
def main() -> None:
    ...
    approaches = _discover_approach2_configs(
