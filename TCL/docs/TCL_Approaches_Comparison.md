...
| Metric                           | Approach 1 | Approach 2        | Approach 4 | Approach 5   |
| -------------------------------- | ---------- | ----------------- | ---------- | ------------ |
| **Intra-Topic Similarity** | 0.87       | **0.929**   | 0.790      | **0.938**   |
| **Inter-Topic Similarity** | 0.23       | **0.0009**  | 0.331      | **-0.13**   |
| **Separation Score**       | 0.64       | **1024.21** | 0.459      | **-7.36**   |
...
| -------------------- | -------------- | --------------- | --------------- | ------------- |
| **War**        | 0.89           | 0.94            | 0.823           | **0.943**     |
| **Health**     | 0.86           | 0.92            | 0.791           | **0.943**     |
| **Economics**  | 0.84           | 0.91            | 0.756           | **0.938**     |
| **Technology** | 0.88           | 0.93            | 0.812           | **0.941**     |
| **Climate**    | 0.87           | 0.94            | 0.768           | **0.927**     |
| **Average**    | **0.87** | **0.929** | **0.790** | **0.938** |

#### Approach 5 Similarity Extremes

- **Intra-topic similarity:** min = 0.679, max = 0.999
- **Inter-topic similarity:** min = -0.187, max = 0.018

**Pairwise Inter-Topic Similarity (Approach 5):**

| Pair                  | Similarity |
|-----------------------|------------|
| Health-War            | -0.128     |
| Health-Technology     | -0.135     |
| Health-Climate        | -0.119     |
| Health-Economics      | -0.130     |
| War-Technology        | -0.131     |
| War-Climate           | -0.123     |
| War-Economics         | -0.126     |
| Technology-Climate    | -0.127     |
| Technology-Economics  | -0.132     |
| Climate-Economics     | -0.123     |
...
This table summarizes the key hyperparameters for each TCL approach, focusing on window size, stride, and approach-specific grouping/segmentation parameters.

| Approach         | Window Size | Stride | Grouping/Segmentation Parameter                       |
| ---------------- | ----------- | ------ | ----------------------------------------------------- |
| 1 (Baseline)     | Yes         | Yes    | None (fixed day-level only)                           |
| 2 (Group-Based)  | Yes         | Yes    | Fixed group size**or** Max day gap (choose one) |
| 4 (Ruptures)     | Yes         | Yes    | Ruptures: PELT penalty/temp for segmentation          |
| 5 (Entity-Aware) | Yes         | Yes    | Ruptures: PELT penalty/temp for segmentation          |

**Notes:**

- All approaches use window size and stride as core hyperparameters.
- Approach 2 requires either a fixed group size or a max day gap for grouping (not both).
- Approaches 4 and 5 require setting the rupture segmentation temperature/penalty.
- Approach 1 uses only window size and stride (no grouping/segmentation).

---

For detailed configuration and rationale, see the respective approach documentation files.
